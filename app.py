...
def build_forecast_model():
    """
    Trains a simple Linear Regression forecaster on weekly sales.
    No TensorFlow needed — works on all platforms.
    Uses lag features (past 4 weeks) just like LSTM.
    """
    weekly = pd.read_csv('data/weekly_sales.csv')
    revenue = weekly['Revenue'].values

    # Create lag features — same concept as LSTM window
    window = 4
    X, y = [], []
